...
class Config:
    ...
    __secretFilePath = (
        os.path.dirname(os.path.abspath(sys.argv[0])) + "/" + "secret.json"
    )
    __stockDB = "stocks.db"
    ...
    def setupEnvironment(isBacktesting=False):
        return (
            copy.deepcopy(Config.getBrokerSession()),
            Config.getDBCursors(isBacktesting),
        )
    ...
    @staticmethod
    def setConfigValues(index, value):
        Config.__configValues[index] = value
        file = open("config.json", "w")
        status = file.write(json.dumps(Config.__configValues))
        file.close()
        return status
    ...
    def getBrokerSession():
        Config.__loadConfigurations()
        broker = list(Config.__secretValues.keys())[0]
